...
from autogen.agentchat.chat import ChatResult, a_initiate_chats, initiate_chats
...
class InstructionAgent(LLMAgent):
    ...
    def __init__(
        ...
        self.register_reply([Agent, None], InstructionAgent.generate_oai_reply)
        self.register_reply([Agent, None], InstructionAgent.a_generate_oai_reply, ignore_async_in_sync_chat=True)
        ...
        if code_execution_config is not False:
            ...
            if self._code_execution_config.get("executor") is not None:
                ...
                self.register_reply([Agent, None], InstructionAgent._generate_code_execution_reply_using_executor)
            else:
                ...
                self.register_reply([Agent, None], InstructionAgent.generate_code_execution_reply)
        else:
            # Code execution is disabled.
            self._code_execution_config = False

        self.register_reply([Agent, None], InstructionAgent.generate_tool_calls_reply)
        self.register_reply([Agent, None], InstructionAgent.a_generate_tool_calls_reply, ignore_async_in_sync_chat=True)
        self.register_reply([Agent, None], InstructionAgent.generate_function_call_reply)
        self.register_reply(
            [Agent, None], InstructionAgent.a_generate_function_call_reply, ignore_async_in_sync_chat=True
        )
        self.register_reply([Agent, None], InstructionAgent.check_termination_and_human_reply)
        self.register_reply(
            [Agent, None], InstructionAgent.a_check_termination_and_human_reply, ignore_async_in_sync_chat=True
        )
    ...
    def _summary_from_nested_chats(
        ...
        Returns:
            Tuple[bool, str]: A tuple where the first element indicates the completion of the chat, and the second element contains the summary of the last chat if any chats were initiated.
        """
        chat_to_run = InstructionAgent._get_chats_to_run(chat_queue, recipient, messages, sender, config)
        if not chat_to_run:
            return True, None
    ...
    def _process_message_before_send(
        ...
        for hook in hook_list:
            if inspect.iscoroutinefunction(hook):
                continue
            message = hook(
                sender=self, message=message, recipient=recipient, silent=InstructionAgent._is_silent(self, silent)
            )
        return message
    ...
    def send(
        ...
        """
        message = self._process_message_before_send(message, recipient, InstructionAgent._is_silent(self, silent))
    ...
    async def a_send(
        ...
        message = await self._a_process_message_before_send(
            message, recipient, InstructionAgent._is_silent(self, silent)
        )
        # When the agent composes and sends the message, the role of the message is "assistant"
        # unless it's "function".
        valid = self._append_oai_message(message, "assistant", recipient, is_sending=True)
    ...
    def _process_received_message(self, message: Union[Dict, str], sender: Agent, silent: bool):
        ...
        if not InstructionAgent._is_silent(sender, silent):
    ...
    def _reflection_with_llm_as_summary(sender, recipient, summary_args):
        ...
        prompt = InstructionAgent.DEFAULT_SUMMARY_PROMPT if prompt is None else prompt
    ...
    def generate_oai_reply(
        ...
        extracted_response = self._generate_oai_reply_from_client(
            client, messages + self._oai_system_message, self.client_cache
        )
    ...
    async def a_generate_oai_reply(
        ...
        return await asyncio.get_event_loop().run_in_executor(
            ...
            lambda: parent_context.run(
                _generate_oai_reply, self=self, iostream=iostream, messages=messages+self._oai_system_message, sender=sender, config=config
            ),
        )
...
def register_function(
    f: Callable[..., Any],
    *,
    caller: InstructionAgent,
    executor: InstructionAgent,
    name: Optional[str] = None,
    description: str,
) -> None:
